...
import data from '../utils/2_1.json'
...
onMounted(() => {
  selected.value = myRandom(cloneDeep(data), 2)
...
const choose = (a) => {
  ...
  if (currentIndex.value === 2) {
    setTimeout(() => {
      // 第一关第二个环节答完题后计算总的错题数
      store.secondErrorCount = store.secondErrorCount + wrongCount.value
      router.push('/secondTwoEnter')
    }, 1000)
  } else {
    setTimeout(() => {
      // 切换到下一题
...
<template>
  <div style="height: 100%" class="secondOneDetail">
    <div class="title">
      <div class="goHome" @click="goHome">
        <el-icon :title="返回首页"><HomeFilled /></el-icon>
      </div>
      <div class="main-title">少先队史大闯关</div>
    ...
    <el-dialog
      ...
      :show-close="false"
    >
      <span>您当前的答题错题数达到2题，请重新答题(5秒后自动跳转到答题页)</span>
    </el-dialog>
  </div>
</template>
<style lang="less">
